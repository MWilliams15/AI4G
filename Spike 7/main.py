...
def on_key_press(symbol, modifiers):
    ...
    #modify group variables
        ...
        for agent in world.agents:
            agent.modify_variables('alignment', True)

    elif symbol == KEY.X:
        for agent in world.agents:
            agent.modify_variables('alignment', False)

    elif symbol == KEY.C:
        for agent in world.agents:
            agent.modify_variables('cohesion', True)

    elif symbol == KEY.V:
        for agent in world.agents:
            agent.modify_variables('cohesion', False)

    elif symbol == KEY.B:
        for agent in world.agents:
            agent.modify_variables('wander', True)

    elif symbol == KEY.N:
        for agent in world.agents:
            agent.modify_variables('wander', False)
