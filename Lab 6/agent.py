...
class Agent(object):
    ...
    def modify_variables(self,variable,increase):
        if variable is 'radius':
            if increase:
                self.wander_radius += 0.1 * self.bRadius
            else: self.wander_radius -= 0.1 * self.bRadius

        elif variable is 'jitter':
            if increase:
                self.wander_jitter += 0.1 * self.bRadius
            else:
                self.wander_jitter -= 0.1 * self.bRadius

        elif variable is 'dist':
            if increase:
                self.wander_dist += 0.1 * self.bRadius
            else:
                self.wander_dist -= 0.1 * self.bRadius


        print("jitter: {}, dist: {}, radius:{}".format(self.wander_jitter,self.wander_dist,self.wander_radius))
